...
## PHASE 4.5: GENERATE VALIDATION PLAN

**Every task MUST include a validation plan at creation time.** The planning agent determines HOW to validate — the executing agent follows the plan exactly.

### 4.5.1 Detect Validation Type

Analyze what the task changes to determine validation type:

| Task changes... | Validation Type |
|-----------------|----------------|
| UI, pages, components, styles | UI |
| API endpoints, backend logic | API |
| Database schema, migrations | DATABASE |
| Fixes a reported bug | BUG_FIX |
| Infra, Docker, CI/CD | INFRA |
| Docs only | DOCS |

A task may have MULTIPLE validation types (e.g., a feature with UI + API).

### 4.5.2 Evidence Commands Per Type

For each validation type, the planning agent MUST specify concrete commands:

**UI Validation:**
- `browser_take_screenshot` with descriptive filename → `evidence/{NN}/after-{page-name}.png`
- `browser_snapshot` for DOM structure → `evidence/{NN}/dom-{page-name}.md`
- `browser_console_messages` for errors → `evidence/{NN}/console-errors.txt`
- Before/after screenshots when modifying existing UI

**API Validation:**
- `curl -s {endpoint}` → save output to `evidence/{NN}/api-response-{name}.json`
- Test both success and error responses
- Verify response structure and status codes

**DATABASE Validation:**
- Run verification query → save output to `evidence/{NN}/db-verify-{name}.txt`
- Confirm schema changes applied correctly
- Verify data integrity after migration

**BUG_FIX Validation:**
- Capture BEFORE state (failing) → `evidence/{NN}/before-{description}.{ext}`
- Capture AFTER state (passing) → `evidence/{NN}/after-{description}.{ext}`
- Both states are MANDATORY — cannot validate a fix without showing the broken state

**INFRA Validation:**
- Capture command output → `evidence/{NN}/infra-{command-name}.txt`
- Verify service health after changes

**DOCS Validation:**
- Confirm file exists and content is accurate
- Save snippet → `evidence/{NN}/docs-snippet.txt`

**ALL Tasks (universal minimum):**
- Run test suite if exists (`npm test`, `phpunit`, `go test`, etc.) → `evidence/{NN}/test-output.txt`
- Save `git diff` → `evidence/{NN}/changes.diff`

### 4.5.3 Output Format

The planning agent writes a "Required Evidence" section in the task file listing numbered validation steps with exact commands and output paths. Example:

```
### Required Evidence
1. [UI] Screenshot after login page: browser_take_screenshot -> evidence/01/after-login-page.png
2. [API] Test auth endpoint: curl -s localhost:3000/api/auth/login -> evidence/01/api-response-auth.json
3. [TEST] Run suite: npm test -> evidence/01/test-output.txt
4. [DIFF] Capture changes: git diff -> evidence/01/changes.diff
```
...
{Generated by planning agent in Phase 4.5. Each line specifies type, command, and output path.}

1. [{TYPE}] {Description}: {command} -> evidence/{NN}/{filename}
2. [{TYPE}] {Description}: {command} -> evidence/{NN}/{filename}
3. [TEST] Run test suite: {test command} -> evidence/{NN}/test-output.txt
4. [DIFF] Capture changes: git diff -> evidence/{NN}/changes.diff

---

## Verification Evidence

| # | Type | File | Description | Pass/Fail |
|---|------|------|-------------|-----------|

> Table populated during execution. Task CANNOT be marked COMPLETED with an empty table.
...
8. **COMPLETION GATE — Evidence-Based Validation**:
   a. Create evidence folder: `docs/daily/{YYYY-MM-DD}/evidence/{NN}/`
   b. Execute EACH validation step from the "Required Evidence" section
   c. Save ALL artifacts to the evidence folder (use `filename` param for screenshots, bash redirect for text)
   d. Populate the "Verification Evidence" table in the task file with each artifact
   e. **Evaluate the gate:**
      - ZERO evidence rows → task stays `IN_PROGRESS` (cannot complete without evidence)
      - ANY row marked `Fail` → update status to `FAILED`
      - ALL rows marked `Pass` AND at least one row exists → update status to `COMPLETED`
   f. Record results, files modified, and any follow-up tasks
   g. Add execution log entry: `| {HH:MM} | {COMPLETED|FAILED|IN_PROGRESS} | {summary with evidence count} |`
...
├── 01_{task-title}.md                 # task file
├── 02_{task-title}.md                 # task file
└── evidence/
    ├── 01/
    │   ├── before-login-page.png      # before state (bug fixes)
    │   ├── after-login-page.png       # after state
    │   ├── console-errors.txt         # browser console output
    │   ├── api-response-auth.json     # API response
    │   ├── test-output.txt            # test suite results
    │   └── changes.diff               # git diff output
    └── 02/
        ├── after-search-page.png
        ├── dom-snapshot.md
        ├── api-response-search.json
        ├── test-output.txt
        └── changes.diff
```
...
- Use **descriptive names**: `after-login-page.png` NOT `screenshot1.png`
- Use **kebab-case** for all filenames
- Include context: `api-response-auth-success.json` vs `api-response-auth-error.json`
- Keep names under 50 characters

### How to Save Each Type

| Evidence Type | How to Save |
|---------------|-------------|
| Screenshots | `browser_take_screenshot` with `filename` parameter set to evidence path |
| DOM snapshots | `browser_snapshot` with `filename` parameter set to evidence path |
| Console logs | `browser_console_messages` with `filename` parameter set to evidence path |
| API responses | `curl -s {url} > evidence/{NN}/{name}.json` via Bash |
| Test output | `{test command} > evidence/{NN}/test-output.txt 2>&1` via Bash |
| Git diff | `git diff > evidence/{NN}/changes.diff` via Bash |
| DB queries | `{query command} > evidence/{NN}/db-verify-{name}.txt` via Bash |
| Command output | `{command} > evidence/{NN}/{name}.txt 2>&1` via Bash |

### When to Collect

1. **BEFORE implementation** — capture current/broken state (mandatory for BUG_FIX tasks)
2. **AFTER implementation** — capture the fixed/new state
3. **FINAL check** — run tests, capture diff, verify no regressions

### Graceful Degradation

If the dev server cannot be started or a service is unavailable:
- **Do NOT mark as COMPLETED** — mark as `PARTIAL`
- Log the reason: `| {HH:MM} | PARTIAL | Server could not start: {error} |`
- Collect whatever evidence IS possible (diff, test output, code review)
- Note which validation steps were skipped and why
...
1. **No evidence = not complete.** If the Verification Evidence table is empty, the task CANNOT be marked `COMPLETED`. Period. An empty table blocks completion regardless of any other claim.

2. **Saved file = proof.** Evidence must be a file saved to disk in the `evidence/{NN}/` folder. Use the `filename` parameter for Playwright tools, or bash redirect (`>`) for command output. "I took a screenshot" without a saved file is NOT evidence.

3. **Claimed output is not evidence.** Pasting terminal output into the task file is NOT evidence. The raw artifact must be saved to disk as a separate file. The Verification Evidence table references the file path.

4. **Before state is mandatory for bug fixes.** A BUG_FIX task requires BOTH before (broken) and after (fixed) evidence. You cannot prove you fixed something without showing it was broken. If you forgot to capture the before state, you must reproduce the bug first, capture it, then apply the fix.

5. **Failure is honest.** If evidence shows the task failed (test failures, wrong UI, error responses), mark the task `FAILED`. Do NOT reinterpret failing evidence as passing. Failure is a valid and expected outcome.

6. **Verify evidence content.** After saving a screenshot, read it back to confirm it shows the expected state. After saving API output, read the file to confirm the response is correct. Do not assume the evidence is valid without checking.

7. **No self-certification.** The phrases "I verified this works", "I confirmed the fix", or "all tasks completed successfully" are NOT evidence. Only saved artifacts count. Your claim is worthless without proof on disk.

8. **Server not running = cannot validate UI/API.** If the dev server is not running, attempt to start it. If it fails to start, you CANNOT validate UI or API tasks. Mark as `PARTIAL` with an explanation. Do NOT claim UI/API validation passed without a running server.

9. **Diff is always required.** Every task that changes code MUST have a `changes.diff` file in its evidence folder. This is the universal minimum evidence. No diff = no proof of work.
